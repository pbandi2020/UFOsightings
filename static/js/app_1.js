...
//New function that is written to handle user interaction
function handleClick() {
    //Grab the datetime value from the filter
    let date = d3.select("#datetime").property("value");
    let filteredData = tableData;
    //conditional filering of data
    if (date) {
    ...
    //Rebuild the table using the filtered table
    //@Note: if no filter is applied then filerdata will 
